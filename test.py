...
for i in range(0, 360, 25):
    ...
    rec_lines, rec_intersections = find_possible_rectangles_lines(merged, corners_set, input_image, debug=False)

    if rec_intersections:
        rec_img = draw_rectangle(rotated, rec_intersections)
        plt.title(f"checking rectangle in rotation: {i}")
        plt.imshow(rec_img, cmap='gray')
        plt.axis('off')
        plt.show()

    else:
        print(f"no rectangles found in: {i}")
